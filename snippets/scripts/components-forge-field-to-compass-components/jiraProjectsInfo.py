...
def get_software_projects_with_pagination(
    domain_name, user_name, api_token, start_at, max_results
):
    ...
    while not isLast:
        try:
            url = (
                domain_name
                + f"/rest/api/3/project/search?type=software&startAt={offset}&maxResults={max_results}&orderBy=key&action=view"
            )
            ...
            if response.ok:
                ...
                print(
                    f"Unexpected HTTP response code {response.status_code} while retrieving projects for {domain_name}. Please try again."
                )
                quit()

        except HTTPError as http_err:
            print(
                f"Couldn't retrieve projects for {domain_name} due to an HTTP error: {http_err}. Please try again."
            )
            quit()
        except Exception as err:
            print(
                f"Couldn't retrieve projects for {domain_name} due to an error: {err}. Please try again."
            )
            quit()
...
def get_projects_by_type(projects, type):
    filtered_projects_raw_json = [
        project for project in projects if project["projectTypeKey"] == type
    ]
...
def main():
    ...
    projects_raw = get_software_projects_with_pagination(
        DOMAIN_NAME, USER_NAME, API_TOKEN, 0, DEFAULT_PAGE_SIZE
    )
...
if __name__ == "__main__":
    ...
    DOMAIN_NAME = input(
        "Enter your domain (example: https://acme.atlassian.net/):"
    ).strip()
