...
def find_compass_component_in_jira(component_name):
    ...
    filtered_response = [
        component
        for component in response["values"]
...
def update_issue_components(
    issue, jira_component_id_to_remove, compass_component_id_to_add
):
    ...
    components_after = [
        ...
        if component["id"] != jira_component_id_to_remove
    ]
...
def main():
    ...
    if not is_valid_project(PROJECT_KEY):
        print(
            "This script only supports Company Managed Jira Software projects. "
    ...
    if not does_project_have_compass_toggle_on():
        print(
            "Compass Components must be turned on for this project before running this script. Exiting..."
        )
        return
    ...
    for component_id, component in jira_component_to_issues_map.items():
        if len(component["issues"]) > 0:
            print(
                f"Jira Component {component['component']['name']} has {len(component['issues'])} issues"
            )
    ...
    if len(jira_components_to_compass_components_map) > 0:
        print(
            "We found following Compass components with same name as the Jira components:"
        )
    for jira_component in jira_components:
    ...
    jira_components_to_create_in_compass = [
        jira_component
        for jira_component in jira_components
        if jira_component["id"] not in jira_components_to_compass_components_map
    ]

    if len(jira_components_to_create_in_compass) > 0:
        print(
            f"Corresponding Compass components for following Jira components will be created:"
        )
        for jira_component in jira_components_to_create_in_compass:
            print(f"{jira_component['name']}")
        print(
            f"To view newly created Compass components, visit: "
    ...
    for jira_component in jira_components_to_create_in_compass:
        ...
        if compass_component["success"] is False:
            print(
                f"Failed to create compass component for Jira Component"
    ...
    if len(jira_component_to_issues_map) > 0:
        ...
        print(
            "No issues found for any of these Jira components. Nothing to migrate. Exiting..."
        )
    ...
    for component_id, component in jira_component_to_issues_map.items():
        ...
        if component_id not in jira_components_to_compass_components_map:
            print(
                f"Compass component not found for Jira component {component['component']['name']}. "
        ...
        if compass_component is not None:
            for issue in issues:
                ...
                print(
                    f"Updated issue {issue_key} by replacing Jira component {component['component']['name']} with "
...
if __name__ == "__main__":
    ...
    print(
        f"This script is used to migrate Jira components of a given project to Compass components.\n"
